...
class RepoGateEngine:
    ...
    UNAUTHORIZED_PATTERNS = [
        (
            r"\b(?:within\s+\d+\s+(?:hours|days)|48\s*hours|24\s*hours|30\s*days)\b",
            "Unapproved SLA or response turnaround timeframe",
        ),
        (
            r"\b(?:bounty(?: of| up to)?\s*[$]?\d+|[$]?\d+\s*(?:usd|usdc|dollars)?\s*reward)\b",
            "Unapproved financial bounty or reward commitment",
        ),
        (
            r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b",
            "Hardcoded specific contact email without upstream authorization",
        ),
    ]

    def __init__(
        self, github_token: Optional[str] = None, proxies: Optional[Dict[str, str]] = None
    ):
        self.github = GitHubClient(github_token=github_token, proxies=proxies)
        self.token = self.github.token
        self.proxies = self.github.proxies
    ...
    def analyze_pr(self, owner: str, repo: str, pr_number: int) -> Dict[str, Any]:
        ...
        if policy_gate["status"] == "FAIL":
            ...
            for f in policy_gate["findings"]:
                blocking.append(
                    f"Policy Guard violation in {f['file']}: {f['reason']} ({', '.join(f['matches'])})"
                )
    ...
    def _eval_ci_gate(self, base_api: str, pr: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if runs:
                ...
                if in_progress:
                    return {
                        "status": "Submitted",
                        "runs_checked": len(runs),
                        "evidence": "Workflows still queued or in progress",
                    }
                return {
                    "status": "CI Passed",
                    "runs_checked": len(runs),
                    "evidence": f"All {len(runs)} workflows concluded successfully",
                }
        ...
        if combined_resp.status_code == 200:
            ...
            if state == "success":
                return {
                    "status": "CI Passed",
                    "evidence": "Commit status combined state is success",
                }
            elif state == "failure":
                return {
                    "status": "CI Failed",
                    "evidence": "Commit status combined state is failure",
                }
            elif state == "pending":
    ...
    def _eval_duplicate_gate(
        self, owner: str, repo: str, pr: Dict[str, Any], files: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        P0-4 Enhancement:
        Checks:
        ...
        linked_issues = re.findall(
            r"(?:closes|fixes|resolves|issue)\s*#?(\d+)", f"{title} {body}", re.I
        )
        ...
        for iss_num in linked_issues:
            ...
            if s_resp.status_code == 200:
                merged_prs = [
                    item for item in s_resp.json().get("items", []) if item.get("number") != pr_num
                ]
                if merged_prs:
        ...
        for f in files:
            ...
            raw_url = (
                f"https://api.github.com/repos/{owner}/{repo}/contents/{fname}?ref={default_branch}"
            )
        ...
        if words:
            ...
            if s_resp.status_code == 200:
                ...
                merged_dup = [
                    d for d in duplicates if d.get("pull_request", {}).get("merged_at") is not None
                ]
                if not merged_dup:
                    ...
                    if m_resp.status_code == 200:
                        merged_dup = [
                            item
                            for item in m_resp.json().get("items", [])
        ...
        return {
            "status": "CLEAN",
            "reason": "No duplicate historical PR or default branch conflict detected",
        }

    def _eval_issue_gate(
        self, owner: str, repo: str, pr: Dict[str, Any], files: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        P0-5 Enhancement:
        ...
        for f in files:
            ...
            if c_resp.status_code == 200:
                ...
                if commits and not pr.get("merged"):
                    ...
                    author_login = (
                        latest_commit.get("author", {}).get("login")
                        if latest_commit.get("author")
                        else ""
                    )
        ...
        return {
            "status": "ACTIVE",
            "reason": "Linked issue is open and no upstream superseding commits detected",
        }
    ...
    def _eval_scope_gate(self, pr: Dict[str, Any], files: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {"status": "OPTIMAL_SCOPE", "changed_files": changed_files, "diff_lines": total_diff}
    ...
    def evaluate_pr(self, pr_url: str) -> Dict[str, Any]:
        """Convenience method to evaluate PR directly from URL."""
        owner, repo, pr_number = self.parse_pr_url(pr_url)
        return self.analyze_pr(owner, repo, pr_number)

    def generate_markdown_summary(self, report: Dict[str, Any]) -> str:
        """Alias for format_markdown_report."""
        return self.format_markdown_report(report)
...
"""
        ...
        md += f"\n---\n*Report generated by RepoGate Autonomous PR Quality Agent v{__version__}*\n"
        return md
